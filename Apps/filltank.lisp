...
(defun requestContainerParams()
  (let ((result))
	...
	(setq containerCapacity (request-integer-input-with-condition "Enter the container capacity (press ENTER to stop input): " #'(lambda(val)(setq isGreater (> val 0))) "The number should be strictly positive. Please try again"))
...
(defun main()
  ...
  (terpri)
  (let ((tankCapacity (request-integer-input-with-condition "Enter the requested tank capacity (press ENTER to quit): " #'(lambda(val)(let ((isGreater)) (setq isGreater (> val 0)))) "The number should be strictly positive. Please try again")))
    (terpri)
